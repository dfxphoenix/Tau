...
module.exports = {
	...
	async execute(client, message, args) {
		...
		collector.on('collect', async (query) => {
			const response = query.content.toLowerCase().trim();
			...
			if (response === 'cancel') {
				...
				embed.setTimestamp();
				embed.setFooter({ text: language.USED_BY + ` ${message.member.displayName}`, iconURL: `${message.author.displayAvatarURL()}` });
			...
			queue.addTrack(maxTracks[value - 1]);
