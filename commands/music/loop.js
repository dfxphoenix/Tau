    ...
    execute(client, message, args) {
        ...
        if (args.join('').toLowerCase() === 'queue') {
            if (queue.repeatMode === 1) {
				embed.setAuthor({ name: `${client.user.username} | Loop`, iconURL: `${client.user.displayAvatarURL()}` });
				embed.setColor(client.config.app.color);
				embed.setDescription(language.DISABLE_CURRENT_MUSIC + ` (${client.config.app.px}loop) ${message.author}... ` + language.TRY_AGAIN + ` ❌`);
				embed.setTimestamp();
				embed.setFooter({ text: language.USED_BY + ` ${message.author.username}`, iconURL: `${message.author.displayAvatarURL()}` });
				return message.channel.send({ embeds: [embed] });
			}
            ...
            embed.setAuthor({ name: `${client.user.username} | Loop`, iconURL: `${client.user.displayAvatarURL()}` });
            embed.setColor(client.config.app.color);
            embed.setDescription(language.DISABLE_CURRENT_QUEUE + ` (${client.config.app.px}loop queue) ${message.author}... ` + language.TRY_AGAIN + ` ❌`);
            embed.setTimestamp();
            embed.setFooter({ text: language.USED_BY + ` ${message.author.username}`, iconURL: `${message.author.displayAvatarURL()}` });
            return message.channel.send({ embeds: [embed] });
        }
